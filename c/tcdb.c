...
int ei_error(ei_x_buff *x, const char *error, char** res, int res_size) {
  if (x) ei_x_free(x);
  // new x
  //ei_x_buff *x;
...
/*  int sz;
  ...
  if (vals) {
    ...
    for (j=0; j<tclistnum(vals); j++) {
      ...
      if (sz == value_size && memcmp(value_tmp, value, sz) == 0) {
        tclistdel(vals);
        return true;
      }
    }
...
static int control(ErlDrvData handle, unsigned int command, char* buf, int count, char** res, int res_size) {
  ...
  switch (command) {
    ...
    case PUTKEEP:
      ...
      if (sz <= MAX_VALUE_SIZE) {
        ...
        if (command == PUT)
          ...
          rs = tcbdbputkeep(bdb, &key[0], key_size, &value[0], value_size);
      } else {
        ...
        if (p) {
          ...
          if (command == PUT)
            ...
            rs = tcbdbputkeep(bdb, &key[0], key_size, p, value_size);
          driver_free(p);
        } else
    ...
    case READ:
      // Read({Key::binary(), ValInc::boolean()})
      ei_get_type(buf, &index, &tp, &sz);
      if (tp != ERL_SMALL_TUPLE_EXT || sz != 2)
        return ei_error(&x, "invalid_argument", res, res_size);
      ei_decode_tuple_header(buf, &index, &sz);
      ...
      if (rs) {
        int inc_val, key_size, val_size;
        const char *key = tcbdbcurkey3(cur, &key_size);

        ei_get_type(buf, &index, &tp, &sz);
        if (tp != ERL_ATOM_EXT) {
          tcbdbcurdel(cur);
          return ei_error(&x, "no records found", res, res_size);
        }
        ei_decode_boolean(buf, &index, &inc_val);
        if (!inc_val) {
          ei_x_encode_tuple_header(&x, 2);
          ei_x_encode_atom(&x, "ok");
          ei_x_encode_binary(&x, key, key_size);
        } else {
          const char *val = tcbdbcurval3(cur, &val_size);
          ei_x_encode_tuple_header(&x, 3);
          ei_x_encode_atom(&x, "ok");
          ei_x_encode_binary(&x, key, key_size);
          ei_x_encode_binary(&x, val, val_size);
        }
        tcbdbcurdel(cur);
      } else {
        tcbdbcurdel(cur);
